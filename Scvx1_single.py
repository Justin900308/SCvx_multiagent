...
P_des = np.array(([[8, 8], [0, 0], [0, 8], [8, 0]]))
P_ini = np.array(([[0, 0], [8, 8], [8, 0], [0, 8]]))
...
for i in range(T):  # obs centers
    obs_center[i, :] = np.array([5 - 0.0 * i, 5 + 0.0 * i, 10 + 0.0 * i, 10 + 0.00 * i])
...
def trajectory_gen(P_des, P_ini, obs_center, R, T, Num_agent, Num_obs):
    ...
    X[:, 0] = np.array([P_ini[0, 0], P_ini[0, 1],
                        P_ini[1, 0], P_ini[1, 1],
                        P_ini[2, 0], P_ini[2, 1],
                        P_ini[3, 0], P_ini[3, 1]])  # Initial state
    ...
    for interation in range(30):
        ...
        constraints = [d[0:2, 0] == np.zeros(n * 1)]
        constraints.append(d[2:4, 0] == np.zeros(n * 1))
        constraints.append(d[4:6, 0] == np.zeros(n * 1))
        constraints.append(d[6:8, 0] == np.zeros(n * 1))
        E = np.eye(n)
        ...
        for i in range(N - 1):
            ...
            constraints.append(cp.abs(w1[0, i]) <= r_default)
            constraints.append(cp.abs(w1[1, i]) <= r_default)
            j = 1
            # constraints.append(w2[:, i] <= r_default)
            # constraints.append(-r_default <= w2[0, i])
            # constraints.append(-r_default <= w2[1, i])
            constraints.append(cp.abs(w2[0, i]) <= r_default)
            constraints.append(cp.abs(w2[1, i]) <= r_default)
            j = 2
            # constraints.append(w3[:, i] <= r_default)
            # constraints.append(-r_default <= w3[0, i])
            # constraints.append(-r_default <= w3[1, i])
            constraints.append(cp.abs(w3[0, i]) <= r_default)
            constraints.append(cp.abs(w3[1, i]) <= r_default)
            j = 3
            # constraints.append(w4[:, i] <= r_default)
            # constraints.append(-r_default <= w4[0, i])
            # constraints.append(-r_default <= w4[1, i])
            constraints.append(cp.abs(w4[0, i]) <= r_default)
            constraints.append(cp.abs(w4[1, i]) <= r_default)
            ...
            for k in range(Num_obs + Num_agent - 1):
                ...
                if k != Num_obs + 1:# agent 1 obs constraints
                    constraints.append(
                        2 * R_list[k] - cp.norm(X[2:4, i] - Current_obs_list[k, :], 2) -
                        (X[2:4, i] - Current_obs_list[k, :]).T @
                        (X[2:4, i] + d[2:4, i] - Current_obs_list[k, :]) /
                        cp.norm(X[2:4, i] - Current_obs_list[k, :], 2) <= s2[(N) * count_agen1 + i, 0]
                ...
                if k != Num_obs + 3:# agent 3 obs constraints
                    constraints.append(
                        2 * R_list[k] - cp.norm(X[6:8, i] - Current_obs_list[k, :], 2) -
                        (X[6:8, i] - Current_obs_list[k, :]).T @
                        (X[6:8, i] + d[6:8, i] - Current_obs_list[k, :]) /
                        cp.norm(X[6:8, i] - Current_obs_list[k, :], 2) <= s2[(N) * count_agen1 + i, 0]

                    )
                    count_agen3 = count_agen3 + 1

        # Terminal condition
        constraints.append(X[0:2, N - 1] + d[0:2, N - 1] == np.array([P_des[0, 0], P_des[0, 1]]))
        constraints.append(X[2:4, N - 1] + d[2:4, N - 1] == np.array([P_des[1, 0], P_des[1, 1]]))
        constraints.append(X[4:6, N - 1] + d[4:6, N - 1] == np.array([P_des[2, 0], P_des[2, 1]]))
        constraints.append(X[6:8, N - 1] + d[6:8, N - 1] == np.array([P_des[3, 0], P_des[3, 1]]))
        # Define the problem
        ...
        for i in range(Num_agent):
            if i == 0:
                ...
                plt.plot(X[2:3, :], X[3:4, :], 'b.', markersize=2)
            elif i == 2:
                plt.plot(X[4:5, :], X[5:6, :], 'b.', markersize=2)
            elif i == 3:
                plt.plot(X[6:7, :], X[7:8, :], 'b.', markersize=2)
            plt.xlim((-10, 20))
            plt.ylim((-10, 20))
            plt.pause(0.001)
        # plt.clf()
...
for i in range(int(T)):
    ...
    for j in range(Num_agent):
        ...
        if j == 0:
            ...
            plt.plot(X[2:3, :i], X[3:4, :i], 'g.', markersize=2)
            plt.plot(X[2:3, i] + x_theta, X[3:4, i] + y_theta, 'g.', markersize=2)
        if j == 2:
            plt.plot(X[4:5, :i], X[5:6, :i], 'r.', markersize=2)
            plt.plot(X[4:5, i] + x_theta, X[5:6, i] + y_theta, 'r.', markersize=2)

        elif j == 3:
            plt.plot(X[6:7, :i], X[7:8, :i], 'g.', markersize=2)
            plt.plot(X[6:7, i] + x_theta, X[7:8, i] + y_theta, 'g.', markersize=2)
