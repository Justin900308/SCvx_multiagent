...
for iteration=1:10
    ...
    X_stack_out=zeros(32,length(X(1,:)));
    X_stack_est=zeros(32,length(X(1,:)));
    ...
    if iteration==1
        ...
        [Cost_1,X_out1]=fetchOutputs(f1); % X_out1 contains X11,X12,X13,X14
        [Cost_2,X_out2]=fetchOutputs(f2);
        [Cost_3,X_out3]=fetchOutputs(f3);
        [Cost_4,X_out4]=fetchOutputs(f4);
        X_stack_est=[X_out1;X_out2;X_out3;X_out4]; %Agent i's estimation of j
        %%

        X_stack_est=[X_out1;X_out2;X_out3;X_out4];
        X_stack_out=zeros(32,length(X(1,:)));
        for i=1:Num_agen
            for j=1:Num_agen
                if i~=j
                    if i==1 % 1's neighbors are 2 and 4
                        X_stack_out((i-1)*Num_agen*2+2*j-1:(i-1)*Num_agen*2+2*j,:)= ...
                        X_stack_est((i-1)*Num_agen*2+2*j-1:(i-1)*Num_agen*2+2*j,:)+ ...
                            epsilon*(  X_stack_est((2-1)*Num_agen*2+j*2-1:(2-1)*Num_agen*2+j*2,:)+ ...
                            X_stack_est((4-1)*Num_agen*2+j*2-1:(4-1)*Num_agen*2+j*2,:) ...
                            -2*X_stack_est((i-1)*Num_agen*2+2*j-1:(i-1)*Num_agen*2+2*j,:)  );

                    elseif i==2 % 2's neighbors are 1 and 3
                        X_stack_out((i-1)*Num_agen*2+2*j-1:(i-1)*Num_agen*2+2*j,:)= ...
                            X_stack_est((i-1)*Num_agen*2+2*j-1:(i-1)*Num_agen*2+2*j,:)+ ...
                            epsilon*(  X_stack_out((1-1)*Num_agen*2+j*2-1:(1-1)*Num_agen*2+j*2,:)+ ...
                            X_stack_est((3-1)*Num_agen*2+j*2-1:(3-1)*Num_agen*2+j*2,:) ...
                            -2*X_stack_est((i-1)*Num_agen*2+2*j-1:(i-1)*Num_agen*2+2*j,:)  );

                    elseif i==3 % 3's neighbors are 2 and 4
                        X_stack_out((i-1)*Num_agen*2+2*j-1:(i-1)*Num_agen*2+2*j,:)= ...
                            X_stack_est((i-1)*Num_agen*2+2*j-1:(i-1)*Num_agen*2+2*j,:)+ ...
                            epsilon*(  X_stack_out((2-1)*Num_agen*2+j*2-1:(2-1)*Num_agen*2+j*2,:)+ ...
                            X_stack_est((4-1)*Num_agen*2+j*2-1:(4-1)*Num_agen*2+j*2,:) ...
                            -2*X_stack_est((i-1)*Num_agen*2+2*j-1:(i-1)*Num_agen*2+2*j,:)  );

                    elseif i==4 % 4's neighbors are 1 and 3 
                        X_stack_out((i-1)*Num_agen*2+2*j-1:(i-1)*Num_agen*2+2*j,:)= ...
                            X_stack_est((i-1)*Num_agen*2+2*j-1:(i-1)*Num_agen*2+2*j,:)+ ...
                            epsilon*(  X_stack_out((1-1)*Num_agen*2+j*2-1:(1-1)*Num_agen*2+j*2,:)+ ...
                            X_stack_est((3-1)*Num_agen*2+j*2-1:(3-1)*Num_agen*2+j*2,:) ...
                    ...
                    X_stack_out((i-1)*Num_agen*2+j*2-1:(i-1)*Num_agen*2+j*2,:)=X_stack_est((i-1)*Num_agen*2+j*2-1:(i-1)*Num_agen*2+j*2,:);
                end
            end
        end
        ...
        X1=X_stack_out(1:8,:);
        X2=X_stack_out(9:16,:);
        X3=X_stack_out(17:24,:);
        X4=X_stack_out(25:32,:);
        % X1=[X_out1(1:2,:);X_out2(3:4,:);X_out3(5:6,:);X_out4(7:8,:)];
        % X2=[X_out1(1:2,:);X_out2(3:4,:);X_out3(5:6,:);X_out4(7:8,:)];
        % X3=[X_out1(1:2,:);X_out2(3:4,:);X_out3(5:6,:);X_out4(7:8,:)];
        % X4=[X_out1(1:2,:);X_out2(3:4,:);X_out3(5:6,:);X_out4(7:8,:)];
        X_out=[X1(1:2,:);X2(3:4,:);X3(5:6,:);X4(7:8,:)];
    %         hold on
    % 
    % for i=1:Num_agen
    %     hold on
    %     plot(X1((2*i-1),:),X1((2*i),:),'r.')
    %     plot(X_out((2*i-1),:),X_out((2*i),:),'g.')
    % end
    %     ee=5;
        %%
    else
        ...
        f1=parfeval(@traj_gen,2,1,Ad,Bd,X1,X0,u,R_plot,obs_center,final,Laplacian);
        f2=parfeval(@traj_gen,2,2,Ad,Bd,X2,X0,u,R_plot,obs_center,final,Laplacian);
        f3=parfeval(@traj_gen,2,3,Ad,Bd,X3,X0,u,R_plot,obs_center,final,Laplacian);
        f4=parfeval(@traj_gen,2,4,Ad,Bd,X4,X0,u,R_plot,obs_center,final,Laplacian);
        ...
        X4=X_stack_out(25:32,:);
        disp("Cost 1    " + Cost_1)  
        obs_checker(X1,Num_agen,obs_center,R)
        disp("Cost 2    " + Cost_2)
        obs_checker(X2,Num_agen,obs_center,R)
        disp("Cost 3    " + Cost_3)
        obs_checker(X3,Num_agen,obs_center,R)
        disp("Cost 4    " + Cost_4)
        obs_checker(X4,Num_agen,obs_center,R)
    ...
    for i=1:Num_agen
        hold on
        plot(X((2*i-1),:),X((2*i),:),'r.')
        plot(X_out((2*i-1),:),X_out((2*i),:),'r.')
    end
...
for iteration=1:15
    ...
    j=Agent_num;
    % minimize (  100*norm((u((2*j-1):(2*j),:)+w),1) + ...
    %     norm((X((2*j-1):(2*j),:)+d-X0((2*j-1):(2*j),:)),1) +  ...
    %     1000*lambda*sum(sum(abs(v)))  + 1000*lambda*(   sum(sum(max(s,0)))   ))
    minimize (  100*norm((u((2*j-1):(2*j),:)+w-1*V_des((2*j-1):(2*j),:)),1) + ...
        norm((X((2*j-1):(2*j),:)+d-X0((2*j-1):(2*j),:)),1) +  ...
        1000*lambda*sum(sum(abs(v)))  + 1000*lambda*(   sum(sum(max(s,0)))   ))
    ...
    X((2*j-1):(2*j),N)+d(:,N)==final((2*j-1):(2*j));
    ...
    Linear_cost(iteration)=(  100*norm((u((2*j-1):(2*j),:)+w-1*V_des((2*j-1):(2*j),:)),1) + ...
        norm((X((2*j-1):(2*j),:)+d-X0((2*j-1):(2*j),:)),1) +  ...
        1000*lambda*sum(sum(abs(v)))  + 1000*lambda*(   sum(sum(max(s,0)))   ));
    ...
    if Linear_cost(iteration)<=10000
        if abs(delta_L) <= rho0
            r = max(r, 0.05);
            X((2*j-1):(2*j),:) = X((2*j-1):(2*j),:) + d;
            u((2*j-1):(2*j),:) = u((2*j-1):(2*j),:) + w;
...
Linear_cost(iteration)
    if  Linear_cost(iteration)<=1000 && iteration>5
        X_out=X;
        u_out=u;
...
for i=I:I
    for j=1:Num_agen
        hold on
        if j==1
            plot(X((2*j-1),1:i),X((2*j),1:i),'r.')
        elseif j==2
            plot(X((2*j-1),1:i),X((2*j),1:i),'g.')
        elseif j==3
            plot(X((2*j-1),1:i),X((2*j),1:i),'b.')
        elseif j==4
            plot(X((2*j-1),1:i),X((2*j),1:i),'c.')
        end
